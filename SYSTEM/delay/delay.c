...
//V1.3修改说明
//增加了对UCOSII延时的支??
//如果使用ucosII,delay_init会自动设置SYSTICK的??使之与ucos的TICKS_PER_SEC对应.
//delay_ms和delay_us也进行了针对ucos的改??
//delay_us可以在ucos下使??而且准确度很??更重要的是没有占用额外的定时??
//delay_ms在ucos??可以当成OSTimeDly来用,在未启动ucos??它采用delay_us实现,从而准确延??
//可以用来初始化外??在启动了ucos之后delay_ms根据延时的长??选择OSTimeDly实现或者delay_us实现.
//V1.4修改说明 20110929
//修改了使用ucos,但是ucos未启动的时??delay_ms中中断无法响应的bug.
//V1.5修改说明 20120902
...
void delay_init()
{
	...
	reload*=1000000/delay_ostickspersec;		//根据delay_ostickspersec设定溢出时间
												//reload??4位寄存器,最大??16777216,??2M??约合1.86s左右	
	fac_ms=1000/delay_ostickspersec;			//代表OS可以延时的最少单??   

	SysTick->CTRL|=SysTick_CTRL_TICKINT_Msk;   	//开启SYSTICK中断
	SysTick->LOAD=reload; 						//??/delay_ostickspersec秒中断一??
...
{	 		  	  
	u32 temp;		   
	SysTick->LOAD=(u32)nms*fac_ms;				//时间加载(SysTick->LOAD??4bit)
	SysTick->VAL =0x00;							//清空计数??
	SysTick->CTRL|=SysTick_CTRL_ENABLE_Msk ;	//开始倒数  
	do
	{
